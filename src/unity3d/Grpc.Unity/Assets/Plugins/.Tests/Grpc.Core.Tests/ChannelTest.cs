...
namespace Grpc.Core.Tests
{
    public class ChannelTest
    {
        ...
        {
            var channel = new Channel("localhost", ChannelCredentials.Insecure);
            AsyncAssert.ThrowsAsync(typeof(ArgumentException), async () => await channel.WaitForStateChangedAsync(ChannelState.Shutdown));
            channel.ShutdownAsync().Wait();
        }
        ...
        public void Shutdown_AllowedOnlyOnce()
        {
            var channel = new Channel("localhost", ChannelCredentials.Insecure);
            channel.ShutdownAsync().Wait();
            AsyncAssert.ThrowsAsync(typeof(InvalidOperationException), async () => await channel.ShutdownAsync());
        }
        ...
        public async Task OperationsThrowAfterShutdown()
        {
            var channel = new Channel("localhost", ChannelCredentials.Insecure);
            await channel.ShutdownAsync();
            AsyncAssert.ThrowsAsync(typeof(ObjectDisposedException), async () => await channel.WaitForStateChangedAsync(ChannelState.Idle));
            Assert.Throws(typeof(ObjectDisposedException), () => { var x = channel.ResolvedTarget; });
            AsyncAssert.ThrowsAsync(typeof(TaskCanceledException), async () => await channel.ConnectAsync());
        }
    }
}
